...
namespace BasketService.Model.Services
{
    public class BasketService : IBasketService
    {
        ...
        private readonly RabbitMQBus<BasketCheckOutMessage> _rabbitMQBus;

        public BasketService(BasketDataBaseContext context, IMapper mapper, RabbitMQBus<BasketCheckOutMessage> rabbitMQBus)
        {
       ...
       .SingleOrDefault(p => p.UserId == anonymousId);
        ...
        public ResultDto CheckoutBasket(CheckoutBasketDto checkoutBasket, IDiscountService discountService)
        {
            ...
            var message = mapper.Map<BasketCheckOutMessage>(checkoutBasket);

            message.basketItemMessages = basket.Items.Select(s => new BasketItemMessage
            {
                BasketItemId=s.Id,
            ...
            message.TotalPrice = basket.Items.Sum(x => x.Product.UnitPrice * x.Quantity);
            ...
            _rabbitMQBus.SendMessage(message, QueNames.BasketCheckOut);
